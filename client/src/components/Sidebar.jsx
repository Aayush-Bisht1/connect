...
const Sidebar = () => {
  ...
  const {isloadingMatches, matches,getMatches} = useMatchStore();
  const toggleSidebar = () => {
    setIsOpen(!isopen);
  };
  useEffect(() => {
            ...
            {isloadingMatches ? (
              ...
              matches.map((match) => (
                <Link key={match._id} to={`/chat/${match._id}`}>
                  <div className="flex items-center mb-3 cursor-pointer hover:bg-pink-50 p-2 rounded-lg transition-colors duration-300">
                    <img
                      src={match.image || avatar}
                      alt="User avatar"
                      className="size-11 object-cover rounded-full mr-3 border-2 border-pink-300"
                    />
